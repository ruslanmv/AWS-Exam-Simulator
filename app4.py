...
import time
from utils4 import display_report
import json
import tempfile
...
def finish_exam(temp_report_path):
    ...
    report_json_content, home_button_visible = display_report(report_json) # Call display_report and get JSON content
    ...
    return (
        ...
        gr.update(visible=False),  # explanation_text
        report_json_content,  # Return the JSON content for display
        temp_report_path,
...
with gr.Blocks(css="style.css", title="Exam Simulator") as demo:
    ...
    report_display = gr.Code(language='json', visible=False, elem_classes="report-display") # Changed to gr.Code for JSON display
