...
import time
from utils2 import display_report
import json
import tempfile
...
def return_home(exam_mode_state_value, temp_report_path_state_value):
    ...
    if exam_mode_state_value == "exam":
        # Call finish_exam to generate report and reset exam state
        finish_exam_outputs = finish_exam(temp_report_path_state_value)
        report_data_state_update = finish_exam_outputs[-2] # Get report_data_state output from finish_exam
    else:
...
with gr.Blocks(css="style.css", title="Exam Simulator") as demo:
    ...
    # Attach the display_report function to the demo's load event - now called after finish exam
    demo.load(
        fn=display_report, # Changed to directly call display_report
        inputs=report_data_state, # Pass the report data
        outputs=[result_text, home_button] # Output to result_text and home_button - home button will be shown after report
    )
