# app.py updated as reference for candidate training - result_text kept visible in training mode
# app5.py
import gradio as gr
from backend1 import *
import time
from utils5 import generate_html_report_from_json # Import generate_html_report_from_json
import json
import tempfile
...
def start_exam(exam_choice, start_question, audio_enabled, exam_mode, num_questions):
    ...
    return (
        ...
        gr.update(visible=False), #result text # hide result text at start
...
def show_explanation(index):
    ...
    global exam_mode # Accessing global exam_mode variable
    if 0 <= index < len(selected_questions):
        ...
        # Conditionally hide result_text only in exam mode, keep visible in training mode
        result_text_update = gr.update(visible=False) if exam_mode == "exam" else None
        return (
            f"<div class='explanation-text'>{explanation}</div>",
            gr.update(visible=True),  # Show explanation_text
            result_text_update # Conditionally hide result_text based on exam_mode
        )
    else:
...
def handle_answer(index, answer, audio_enabled, current_audio, exam_mode, temp_report_path):
    ...
    if answer is None:
        return "<div class='result-text'>Please select an option before submitting.</div>", None, None, temp_report_path, None # Modified: Added None for question_audio
    ...
    # Modified: Added gr.update(value=None) to stop question_audio
    return result, answer_audio_path, stop_audio, temp_report_path, gr.update(value=None)
...
def return_home(exam_mode_state_value, temp_report_path_state_value):
    """Returns to the home screen and handles exam finish if in exam mode."""
    if exam_mode_state_value == "exam":
        # No need to call display_report here anymore
        report_data_state_update = None # No report to display on return home, report is already displayed by finish_exam
    else:
        report_data_state_update = None # No report to display for training mode


    """Returns to the home screen."""
    return (
        # Home page elements - make visible
        gr.update(visible=True), # title
        gr.update(visible=True), # description
        gr.update(visible=True), # exam_selector
        gr.update(interactive=False, visible=True), # start_button (disable initially until exam selected and make visible)
        gr.update(visible=True), # audio_checkbox
        gr.update(visible=True), # start_question_slider
        gr.update(visible=True), # exam_mode_radio
        gr.update(visible=True), # num_questions_slider

        # Quiz elements - make invisible and reset
        gr.update(visible=False), # question_text
        gr.update(choices=[], visible=False, interactive=False), # choices (reset choices and visibility)
        ...
        gr.update(value=0),      # question_state (reset to 0)
        gr.update(visible=False), # result_text
        gr.update(visible=False), # explanation_text
        gr.update(visible=False), # explain_button
        gr.update(visible=False), # timer_text
        gr.update(value=""),      # result_text - clear any result message
        "",           # temp_report_path_state - reset
        report_data_state_update,  # report_data_state - update if exam mode
        gr.update(visible=False), # temp_report_path_display - hide on home
        gr.update(visible=False) # report_display - hide on home
    )
...
def finish_exam(temp_report_path):
    """Displays the final results using display_report and hides the quiz elements."""
    print("finish_exam function called") # Debugging print
    if temp_report_path:
        try:
            with open(temp_report_path, 'r') as f:
                report_data = json.load(f) # Load report data as JSON object

            # Calculate score - keep this logic
            correct_answers = 0
            ...
            # Update report_data in the file - keep this logic
            with open(temp_report_path, 'w') as f:
                json.dump(report_data, f)
    ...
    # Hide quiz elements and show report
    report_json = json.dumps(report_data) # Convert report_data to JSON string
    html_report_content = generate_html_report_from_json(report_json) # Generate HTML report
    ...
    return (
        ...
        gr.update(visible=True),  # home_button - SHOW in report page
        gr.update(visible=False),  # explanation_text
        html_report_content,  # Return the HTML content for display # UPDATED to HTML
        temp_report_path,
        gr.update(visible=True, value=f"Report file: {temp_report_path}"), # Show report path
        gr.update(visible=True) # ADDED: report_display - Make report_display visible
    )
...
with gr.Blocks(css="style.css", title="Exam Simulator") as demo:
    ...
    start_question_slider = gr.Slider(minimum=0, maximum=50, step=1, label="Select starting question", visible=True, elem_classes="start_question_slider") # Make always visible on home page
    start_button = gr.Button("Start Exam", elem_classes="start_button", interactive=False) # Initially disabled
    ...
    home_button = gr.Button("Return to Home", visible=False, elem_classes="home_button") # Hide in quiz mode, report will have return home
    ...
    temp_report_path_display = gr.Markdown(visible=False, elem_classes="report-path-display") # Added report path display
    report_display = gr.HTML(visible=False, elem_classes="report-display") # Changed to gr.HTML for HTML display # UPDATED to gr.HTML
    ...
    report_data_state = gr.State() # To hold report data for display_report function
    ...
    with gr.Row(): # Added row for report path display
        gr.Column([temp_report_path_display])

    with gr.Row(): # Added row for report display
        gr.Column([report_display])

    # --- Event Handlers ---

    # Show settings after exam selection
    def show_settings(exam_choice):
        return gr.update(interactive=True) # Enable start_button

    # Connect exam selection to display settings section
    exam_selector.change(fn=show_settings, inputs=[exam_selector], outputs=[start_button])
    ...
    answer_button.click(
        fn=handle_answer,
        inputs=[question_state, choices, audio_checkbox, current_audio_state, exam_mode_state, temp_report_path_state],
        outputs=[
            result_text,
            answer_audio,
            current_audio_state,
            temp_report_path_state,
            question_audio # Modified: Added question_audio to outputs
        ]
    )
    next_button.click(fn=handle_next, inputs=[question_state, audio_checkbox, exam_mode_state, temp_report_path_state], outputs=[question_text, choices, question_state, result_text, question_audio, explanation_text, finish_button, next_button, prev_button, temp_report_path_state]) # Pass audio_checkbox instead of audio_enabled
    prev_button.click(fn=handle_previous, inputs=[question_state, audio_checkbox, exam_mode_state, temp_report_path_state], outputs=[question_text, choices, question_state, result_text, question_audio, explanation_text, finish_button, next_button, prev_button, temp_report_path_state]) # Pass audio_checkbox instead of audio_enabled
    ...
    finish_button.click(
        ...
        outputs=[
            ...
            home_button, explanation_text, report_display, temp_report_path_state,
            temp_report_path_display, # Added to outputs
            report_display # Added report_display to outputs
        ]
    )
    ...
    home_button.click(
        fn=return_home,
        inputs=[exam_mode_state, temp_report_path_state], # Pass exam_mode_state and temp_report_path_state to return_home
        outputs=[
            title,
            description,
            exam_selector,
            ...
            explain_button,
            timer_text,
            result_text, # result_text - to reset result text on home page
            temp_report_path_state, # temp_report_path_state - to reset
            report_data_state, # report_data_state - to reset
            temp_report_path_display, # Added to outputs to hide on home
            report_display # Added report_display to outputs to hide on home
        ]
    )
